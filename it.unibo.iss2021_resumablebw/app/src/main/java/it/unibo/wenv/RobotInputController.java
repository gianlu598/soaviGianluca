...
- a collision between the robot and an obstacle { "collision" : "false", "move": "moveForward"}
    ...
    public synchronized void  handleInfo(JSONObject infoJson) {
        ...
        if( infoJson.has("endmove") )        handleEndMove(infoJson);
            ...
            //with ConsoleGui
        else if( infoJson.has("robotcmd") ) handleConsoleGui(infoJson);
    }

    private void handleConsoleGui(JSONObject consoleCmd) {
        String cmd = (String) consoleCmd.get("robotcmd");
        if(cmd.equals("RESUME") ){
            System.out.println("RobotControllerBoundary | ConsoleGui RESUMED the robot");
            robotBehaviorLogic.doBoundaryGoon();
        }else if(cmd.equals("STOP") ){
            System.out.println("RobotControllerBoundary | ConsoleGui STOPPED the robot");
            robotBehaviorLogic.boundaryStep("stop", false);
        }
    }

    private void handleStop(JSONObject infoJson) {
        System.out.println("RobotControllerBoundary | ConsoleGui STOPPED the robot");
        robotBehaviorLogic.boundaryStep("stop", false);
    }

    private void handleResume(JSONObject infoJson) {
        System.out.println("RobotControllerBoundary | ConsoleGui RESUMED the robot");
        robotBehaviorLogic.doBoundaryGoon();
    }
